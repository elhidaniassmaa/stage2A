...
def get_upload_statut():
    with _upload_state_lock:
        return dict(_upload_state)

def update_upload_statut(**kwargs):
    with _upload_state_lock:
...
from backend.reentrainement_routes import router as reentrainement_router
...
def get_notifications(user_id: Optional[int] = Query(None)):
    """
    Retourne les demandes en attente ayant une alerte active (rappel, prioritaire, dépassé),
    avec indicateur si l'alerte a été vue par l'utilisateur (user_id) et si elle est apparue aujourd'hui.
    """
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    vues_set = set()
    if user_id:
        cursor.execute("SELECT demande_id FROM notifications_vues WHERE user_id = ?", (user_id,))
        vues_set = {row["demande_id"] for row in cursor.fetchall()}

    cursor.execute("""
        SELECT id, date_depot, direction_predite, decision_responsable, type_alerte, date_echeance
        FROM demandes
    ...
    for r in rows:
        ...
        date_depot_str = str(r["date_depot"] or "")
        est_aujourdhui = date_depot_str.startswith(today_iso) or date_depot_str.startswith(today_fr)
        if est_aujourdhui:
            total_aujourdhui += 1

        est_vue = r["id"] in vues_set
        if not est_vue:
            total_non_vues += 1
    ...
    return {
        ...
        "total_non_vues": total_non_vues,
        "total_aujourdhui": total_aujourdhui
    }

@app.post("/api/notifications/marquer-vue")
def marquer_notification_vue(demande_id: int = Query(...), user_id: int = Query(...)):
    conn = get_db_connection()
    cursor = conn.cursor()
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cursor.execute("""
        INSERT INTO notifications_vues (demande_id, user_id, date_vue)
        VALUES (?, ?, ?)
        ON CONFLICT(demande_id, user_id) DO UPDATE SET date_vue = excluded.date_vue
    """, (demande_id, user_id, now_str))
    conn.commit()
    conn.close()
    return {"message": "Notification marquée comme vue."}

@app.post("/api/notifications/tout-marquer-vu")
def tout_marquer_vu(user_id: int = Query(...)):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
    ...
    for r in rows:
        cursor.execute("""
            INSERT INTO notifications_vues (demande_id, user_id, date_vue)
            VALUES (?, ?, ?)
            ON CONFLICT(demande_id, user_id) DO NOTHING
        """, (r["id"], user_id, now_str))
    conn.commit()
    conn.close()
    return {"message": "Toutes les notifications ont été marquées comme vues."}


def _build_alertes_list():
    """Construit la liste des alertes actives avec le flag avec_son."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
    ...
    for r in rows:
        ...
        type_alerte = r["type_alerte"]
        avec_son = type_alerte in ("rappel", "prioritaire")
...
@app.get("/api/notifications/alertes")
def get_notifications_alertes():
    """
    Retourne les alertes actives avec le flag avec_son pour déclencher
    le son navigateur (rappel J-15 et prioritaire J-5).
    """
    alertes = _build_alertes_list()
...
@app.get("/api/demandes/upload-status")
def get_upload_status():
    return get_upload_statut()
...
def upload_demandes(file: UploadFile = File(...), user_name: str = Query(...), user_role: str = Query(...)):
    ...
    try:
        ...
        update_upload_statut(
            en_cours=True,
            total=total_lignes,
            traites=0,
            pourcentage=0,
            etape="Initialisation de l'analyse du fichier Excel...",
            message=""
        )
        ...
        for idx, row in df.iterrows():
            traites_actuel = idx + 1
            pct = int((traites_actuel / total_lignes) * 100) if total_lignes > 0 else 0
            update_upload_statut(
                en_cours=True,
                total=total_lignes,
                traites=traites_actuel,
                pourcentage=pct,
                etape=f"Traitement IA & routage demande {traites_actuel}/{total_lignes}..."
            )
        ...
        update_upload_statut(
            en_cours=False,
            etape="Erreur lors de l'importation",
            message=str(e)
        )
...
def create_article(
    ...
    if user_role != "Administrateur":
        raise HTTPException(
            status_code=403,
            detail="Accès refusé : Seul l'Administrateur peut ajouter des articles réglementaires."
        )
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
...
def update_article(
    ...
    if user_role != "Administrateur":
        raise HTTPException(
            status_code=403,
            detail="Accès refusé : Seul l'Administrateur peut modifier des articles réglementaires."
        )
    conn = get_db_connection()
    cursor = conn.cursor()
...
def delete_article(
    ...
    if user_role != "Administrateur":
        raise HTTPException(
            status_code=403,
            detail="Accès refusé : Seul l'Administrateur peut supprimer des articles réglementaires."
        )
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM articles WHERE id = ?", (id,))
...
@app.get("/api/rapports-historique/{id}")
def get_rapport_historique(id: int):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, nom, type_rapport, date_debut, date_fin, date_generation, genere_par, donnees_json FROM rapports_historique WHERE id = ?", (id,))
    row = cursor.fetchone()
    conn.close()
    if not row:
        raise HTTPException(status_code=404, detail="Rapport non trouvé.")
    res = dict(row)
    if res.get("donnees_json"):
        try:
            res["donnees_json"] = json.loads(res["donnees_json"])
        except Exception:
            pass
    return res
